...
public class JournalServiceImpl implements JournalService {
    ...
    @Override
    public Collection<Journal> getLatestJournals() {
        Calendar cal = Calendar.getInstance();
        cal.add(Calendar.DATE, -1);
        Timestamp start = new Timestamp(cal.getTimeInMillis());
        Timestamp end = new Timestamp(DateUtils.createNow().getTimeInMillis());

        // get latest journal based on yesterday and current time
        Collection<Journal> latestJournals = journalRepository.findByPublishDateBetween(start, end);
        if (latestJournals == null) {
            latestJournals = Collections.emptyList();
        }
